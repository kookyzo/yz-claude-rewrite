...
export default function Register() {
  ...
  const handleGetPhone = async (e) => {
    ...
    try {
      ...
      if (res.code === 200 && res.data?.phoneNumber) {
        ...
        Taro.showToast({ title: '手机号获取成功', icon: 'success', duration: 1500 })
      } else {
        Taro.showToast({ title: res.message || '获取手机号失败', icon: 'none', duration: 2000 })
      }
    } catch {
      Taro.hideLoading()
  ...
  const handleSelectAll = () => {
    ...
    setAgreements({ privacy: newState, marketing: newState, analysis: newState })
  ...
  const handleSubmit = async () => {
    ...
    if (!checkAge(birthday)) {
      Taro.showToast({ title: '您必须年满18周岁才能注册', icon: 'none', duration: 2000 })
      return
    }
    ...
    try {
      ...
      const nickname = `${lastName}${firstName}`.trim() || '微信用户'
      ...
      if (res.code === 200) {
        ...
        Taro.showToast({ title: res.message || '注册失败', icon: 'none', duration: 2000 })
      }
    } catch {
      Taro.showToast({ title: '注册失败，请重试', icon: 'none', duration: 2000 })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <View className={styles.registerContainer}>
      ...
      <View className={styles.registerContent} style={{ marginTop: `${topOffset}px` }}>
        {/* Section Title */}
        ...
        <View className={styles.nameRow}>
          <View className={styles.nameItem}>
            ...
            <View className={`${styles.inputWrapper} ${errors.lastName ? styles.error : ''}`}>
              <Input
          ...
          <View className={styles.nameItem}>
            ...
            <View className={`${styles.inputWrapper} ${errors.firstName ? styles.error : ''}`}>
              <Input
                className={styles.formInput}
                type='text'
                placeholder='请填写名'
        ...
        <View className={styles.genderSection}>
          <View className={styles.genderSectionRow}>
            <Text className={`${styles.formLabel} ${styles.genderLabelInline}`}>称谓</Text>
            <View className={styles.genderOptions}>
              {(['mr', 'ms', 'other'] as GenderType[]).map((g) => (
                <View key={g} className={styles.genderOption} onClick={() => setGender(g)}>
        ...
        <View className={styles.phoneSection}>
          ...
          <View className={`${styles.phoneInputWrapper} ${errors.phone ? styles.error : ''}`}>
            <Text className={styles.requiredStar}>*</Text>
            <Input
        ...
        <View className={styles.agreementsSection}>
          {/* 隐私声明（必选） */}
          <View className={styles.agreementItem} onClick={() => handleAgreementChange('privacy')}>
            ...
            <View className={styles.agreementText}>
              <Text>* 我同意 Y.ZHENG 依照隐私声明使用收集我的个人信息，亦允许第三方依此存储和处理相同内容。</Text>
              <Text className={styles.requiredText}>(必选)</Text>
            </View>
          </View>

          {/* 营销信息 */}
          <View className={styles.agreementItem} onClick={() => handleAgreementChange('marketing')}>
            <View className={styles.agreementCheckbox}>
              <Image
                className={styles.agreementCheckboxIcon}
            ...
            <View className={styles.agreementText}>
              <Text>我同意收取微信，短彩信，电话，邮寄，电邮等一般营销信息。我了解 Y.ZHENG 隐私权中心可提供设置协助。</Text>
            </View>
          </View>

          {/* 分析个人信息 */}
          <View className={styles.agreementItem} onClick={() => handleAgreementChange('analysis')}>
            <View className={styles.agreementCheckbox}>
              <Image
                className={styles.agreementCheckboxIcon}
            ...
            <View className={styles.agreementText}>
              <Text>我同意 Y.ZHENG 依照隐私声明使用分析个人信息，以此建立个人档案与个性化互动营销。</Text>
            </View>
          </View>
